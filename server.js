...
app.use(
  cors({
    origin: (origin, callback) => {
      // Allow requests from localhost and any subdomain of localhost
      console.log("CORS Origin:", origin) // Debug log
      if (!origin || origin === "http://localhost:5173" || origin.match(/^http:\/\/[a-zA-Z0-9-]+\.localhost:5173$/)) {
        callback(null, true)
      } else {
...
app.post("/api/signup", async (req, res) => {
  try {
    ...
    if (existingShops.length > 0) {
      return res.status(400).json({
        message: `Shop name(s) already exist: ${existingShops.map((shop) => shop.name).join(", ")}`,
      })
    }
...
app.post("/api/signin", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id, username: user.username }, JWT_SECRET, { expiresIn: tokenExpiry })
    ...
    res.cookie("authToken", token, {
      httpOnly: false, // Changed to false so we can access it from frontend for debugging
      secure: false,
      sameSite: "lax",
      maxAge: cookieExpiry,
      domain: ".localhost", // This should work for all *.localhost domains
      path: "/",
    })
...
app.get("/api/shop/:shopName", verifyToken, async (req, res) => {
  try {
    ...
    const shop = await Shop.findOne({ name: shopName }).populate("owner", "username")
